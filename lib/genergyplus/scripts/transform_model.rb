            ...
            roof_and_ceiling_objects.each{ |object_type|
                ...
                object_array.each{ |object|
                    ...
                    when "ceiling:adiabatic"
                        warn "'#{obejct.name}' is already adiabatic. Construction was changed anyway."
                    when "floor:adiabatic"
                        warn "'#{obejct.name}' is already adiabatic. Construction was changed anyway."
                    else
                        warn "'#{object.name}' could not be made adiabatic because it is a '#{object.name.capitalize}'. Construction was changed anyway."
                    end
                    object["construction name"] = options["assign construction"].name if options
                }
            }
            return self
        end        
